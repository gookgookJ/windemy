  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {user ? (
              <div className="flex items-center space-x-2">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className="lg:hidden inline-flex sm:inline-flex items-center justify-center"
        ...
        {/* Mobile/Tablet Menu - Compact UI/UX */}
        {isMenuOpen && (
          <div className="lg:hidden border-t border-border bg-white/95 backdrop-blur-md w-screen relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw]">
            <div className="max-w-7xl mx-auto px-4 py-4 space-y-4">
              ...
              <nav className="space-y-1">
                {isMyPageRoute ? (
                  // MyPage specific menu items with compact styling
                  <>
                    <div className="grid gap-2">
                    {myPageMenuItems.map((item, index) => (
                        <div key={item.name} className="animate-fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
                          {item.external ? (
                            <button
                              className="w-full text-left flex items-center justify-between p-3 rounded-lg bg-gradient-to-r from-muted/20 to-muted/5 hover:from-primary/5 hover:to-primary/10 border border-transparent hover:border-primary/10 transition-all duration-200 font-medium text-foreground hover:text-primary"
                              onClick={() => {
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-muted/20 to-muted/40 group-hover:from-primary/10 group-hover:to-primary/20 transition-all duration-200">
                            ...
                            <Link
                              to={item.href}
                              className="flex items-center justify-between p-3 rounded-lg bg-gradient-to-r from-muted/20 to-muted/5 hover:from-primary/5 hover:to-primary/10 border border-transparent hover:border-primary/10 transition-all duration-200 font-medium text-foreground hover:text-primary"
                    ...
                    <div className="pt-3 mt-3 border-t">
                      <div className="animate-fade-in">
                        <button
                          className="w-full text-left flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-destructive/5 to-red-50 hover:from-destructive/10 hover:to-red-100 border border-destructive/10 hover:border-destructive/20 transition-all duration-200 font-medium text-destructive hover:text-destructive"
                          onClick={handleSignOut}
                        >
                          <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-destructive/10 to-destructive/20 group-hover:from-destructive/20 group-hover:to-destructive/30 transition-all duration-200">
                  ...
                  // Regular navigation items with compact styling
                  <>
                    <div className="grid gap-2">
                              ...
                              <div className="mt-1 ml-3 space-y-1">
                                {item.submenu.map((subItem, subIndex) => (
                                  <Link
                                    key={subItem.name}
                                    to={subItem.href}
                                    className="block text-xs text-muted-foreground hover:text-primary transition-colors duration-200 py-1.5 px-3 rounded hover:bg-muted/20"
                                    onClick={() => setIsMenuOpen(false)}
                                    style={{ animationDelay: `${(index * 0.05) + (subIndex * 0.02)}s` }}
                                  >
                                    {subItem.name}
                                  </Link>
                                ))}
                    ...
                    {user && isAdmin && (
                      <div className="pt-3 mt-3 border-t">
                        <div className="animate-fade-in">
                          <Link 
                            to="/admin" 
                            className="flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-destructive/5 to-red-50 hover:from-destructive/10 hover:to-red-100 border border-destructive/10 hover:border-destructive/20 transition-all duration-200 font-medium text-destructive hover:text-destructive"
                            onClick={() => setIsMenuOpen(false)}
                          >
                            <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-br from-destructive/10 to-destructive/20 group-hover:from-destructive/20 group-hover:to-destructive/30 transition-all duration-200">
                ...
                <div className="pt-3">
                  <div className="animate-fade-in">
                    <div className="grid grid-cols-2 gap-3">
                      <Button 
                        variant="outline" 
                        className="h-10 text-sm font-medium rounded-lg border-2 hover:border-primary/50 transition-all duration-200"
                        onClick={() => {
                          setAuthModalTab('signin');
                          setIsAuthModalOpen(true);
                          setIsMenuOpen(false);
                      ...
                      <Button 
                        variant="default" 
                        className="h-10 text-sm font-medium rounded-lg bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 transition-all duration-200"
                        onClick={() => {
                          setAuthModalTab('signup');
                          setIsAuthModalOpen(true);
                          setIsMenuOpen(false);
