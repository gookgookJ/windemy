...
const CourseDetail = () => {
  ...
  const fetchInstructorInfo = async (instructorId: string) => {
    try {
      ...
      if (row) {
        setInstructorInfo({
          full_name: (row.full_name && row.full_name.trim()) || '강사',
          instructor_bio: (row.instructor_bio && row.instructor_bio.trim()) || '',
          instructor_avatar_url: (row.instructor_avatar_url && row.instructor_avatar_url.trim()) || null,
        });
      } else {
