...
DROP POLICY IF EXISTS "Admins can access all course files" ON storage.objects;
...
CREATE POLICY "Section files viewable by enrolled students"
...
USING (
  ...
  EXISTS (
    ...
    AND cs.attachment_url IS NOT NULL
    AND name LIKE '%' || cs.attachment_url || '%'
  )
);

CREATE POLICY "Instructors can manage their section files"
